...
func main() {
	...
	// 3. 测试Token颁发功能
	deliverResp, err := client.DeliverTokenByRPC(context.Background(), &auth.DeliverTokenReq{
		UserId: 1001,
	})
	if err != nil {
	...
	// 4. 测试Token验证功能
	verifyResp, err := client.VerifyTokenByRPC(context.Background(), &auth.VerifyTokenReq{
